...
def get_snappshop_prices(url):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/114.0.0.0 Safari/537.36', 'Referer': 'https://snappshop.ir/'}
    try:
        response = requests.get(url, headers=headers, timeout=20 )
        if response.status_code == 403: return "403"
        return {"Default": None} # اسنپ‌شاپ نیاز به پروکسی یا هاست داخلی دارد
    except: return {}
    ...
    for product in PRODUCTS:
        ...
        for color, current_price in prices.items():
            ...
            if old_price:
                ...
                updates.append(f"✅ {name} ({color}): {current_price:,} تومان")
            
            new_history[key] = current_price

    # در اولین اجرا یک گزارش کلی بفرست
    if updates and not history:
        report = "🚀 *ربات مانیتورینگ با موفقیت فعال شد!*\n\nلیست قیمت‌های شناسایی شده:\n" + "\n".join(updates[:20]) # محدودیت تعداد خط در تلگرام
